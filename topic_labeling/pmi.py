...
class PMICalculator(object):
    ...
    def from_texts(self, docs, docs2, labels):
        """
        Parameter:
        -----------
        ...
        numpy.ndarray: #word x #label
            the pmi matrix
        """
        # d2w = self._d2w_vect.fit_transform(list(map(lambda sent: ' '.join(sent),
        #                                        docs)))
        d2w = self._d2w_vect.fit_transform(docs)

        # save it to avoid re-computation
        self.d2w_ = d2w

        d2l = self._d2l_vect.transform(docs2, labels)
