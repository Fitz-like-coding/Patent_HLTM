...
class QueryExpansion(object):
    ...
    def initialize(self, processed_texts = [], vobs = None):
        ...
        if vobs != None:
            self.tf_vectorizer = CountVectorizer(vocabulary = vobs)
        else:
            self.tf_vectorizer = CountVectorizer()
        self.vectors = self.tf_vectorizer.fit_transform(processed_texts).toarray() + 1e-20
        self.ptMc = self.vectors.sum(0)/(self.vectors.sum())
        temp = self.vectors.sum(1)
        temp[temp==0] = 1
        ptMd = self.vectors/(temp.reshape((-1,1)))
        self.dlm = self.alpha * self.ptMc + (1 - self.alpha) * ptMd
    ...
    def suggest(self, processed_query, num_words = 100, rule="AND", mode="KLD"):
        ...
        if mode == "REL":
            ...
            ptd_2 = np.array(self.dlm)[np.argsort(pqd)[:-top_docs - 1:-1]]
            ptq = (ptd_2 * pqd_2.reshape((-1,1))).sum(0)
            if ptq.sum() != 0:
                ptq = ptq / ptq.sum() # normalize
            else:
                ptq = np.zeros(self.dlm.shape[1]) + 1 / self.dlm.shape[1]
            ...
            top_scores = ptq[np.argsort(ptq)[:-num_words - 1:-1]]
        ...
        if mode == "FRE":
            ...
            top_scores = temp[np.argsort(temp)[:-num_words - 1:-1]]
        ...
        return top_words, top_scores, np.argsort(pqd)[:]
...
if __name__ == '__main__':
    ...
    top_words, top_scores, top_docs = qe.suggest(processed_query, num_words=100, mode="FRE")
    print (top_words, top_docs)
    top_words, top_scores, top_docs = qe.suggest(processed_query, num_words=100, mode="REL")
    print (top_words, top_docs)
    top_words, top_scores, top_docs = qe.suggest(processed_query, num_words=100, mode="KLD")
    print (top_words, top_docs)